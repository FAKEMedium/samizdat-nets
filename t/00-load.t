use strict;
use warnings;
use Test::More;
use_ok('Samizdat::Model::Nets');
use_ok('Samizdat::Controller::Nets');
use_ok('Samizdat::Plugin::Nets');
use YAML::XS qw(LoadFile);
use File::Spec;
my ($d) = grep { -d } map { File::Spec->catdir($_, 'Samizdat','resources') } @INC;
ok($d, 'resources dir is on @INC');
my $schema = eval { LoadFile(File::Spec->catfile($d,'settings','nets','schema.yml')) };
ok(ref $schema eq 'HASH', 'nets settings schema loads')
  and is($schema->{'x-samizdat-audience'}, 'operator', 'audience is operator');
ok(-d File::Spec->catdir($d,'templates','nets'), 'nets templates ship');
ok(scalar(glob(File::Spec->catfile($d,'migrations','pg','*-nets','*','up.sql'))), 'nets pg migration ships');
done_testing;
